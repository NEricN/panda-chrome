    ...
    var tuneInto = function(station, cb) {
        ...
        cleanUp(function() {
            $.post(baseURL+"/listen", JSON.stringify({station: station, gcm: gcmId}), function(data) {
                ...
                chrome.storage.local.set({state: state, tune: curStation, artist: artist, song: song, album: album}, function() { cb({artist: artist, song: song, album: album}); });
            }).fail(function(blarg) {
                cb({error: "Station Not Found"});
            });
        });
    }
